...
def transcribe_audio(file_url, file_name, speaker=None):
    ...
    print("speaker & file_name", speaker, file_name)
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            result = transcribe_audio(file_url, file.filename, speaker)
            # Expecting transcript output in the "output" field
            transcript = result.get("output", {}) if result else {}
            return render_template('transcript.html', transcript=transcript, file_name=file.filename, file_url=file_url)
